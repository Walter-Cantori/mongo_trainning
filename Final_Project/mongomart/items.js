...
function ItemDAO(database) {
        ...
        */
        this.db.collection('item').aggregate([
                ...
                for(var i = 0; i < categories.length; i++){
                    category['num'] = categories[i]['num'] + category['num']
                }
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
         ...
         category = category == 'All' ? /w*/ : category
         this.db.collection('item').find({"category": category}).sort({_id: 1}).skip(skip).limit(itemsPerPage).toArray()
            .then(items => {
                callback(items);
            })
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         */
        var skip = page * itemsPerPage
        this.db.collection('item').find({$text: {$search:query}}).sort({_id: 1}).skip(skip).limit(itemsPerPage).toArray()
            .then(items => {
                callback(items);
            })
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        this.db.collection('item').find({$text: {$search:query}}).count()
            .then(numItems => {
                callback(numItems);
            })
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection('item').find({_id: id}).toArray()
            .then(item =>  {
                callback(item[0])
            })
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        this.db.collection("item").update({_id: itemId}, {$push: {reviews: reviewDoc}})
            .then(result => { callback(reviewDoc) })
